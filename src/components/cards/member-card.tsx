  ...
  return (
    <article className="paper-card rounded-2xl p-4 transition hover:-translate-y-1">
      <div className="flex items-center gap-3">
        <Avatar name={displayName} src={user.profileImage ?? undefined} />
        <div>
          <p className="font-semibold">{displayName}</p>
          <p className="text-sm text-black/65 dark:text-white/70">@{handle}</p>
        </div>
      </div>
      <p className="mt-3 text-sm text-black/75 dark:text-white/80">
        {user.bio}
      </p>
